...
try:
    ...
    class KenyaCropCalendar:
        ...
        def get_agricultural_advice(self, crop, stage, language='en'):
            advice_db = {
                'maize': {
                    'flowering': {
            ...
            return advice_db.get(crop, {}).get(stage, {}).get(language, "Monitor your crops carefully.")
...
def show_dashboard_tab(farmer):
    ...
    with col2:
        st.metric(
            ...
            value=len(expected_blooms),
            delta="+2 this week" if len(expected_blooms) > 0 else "None"
        )
    ...
    with col_left:
        ...
        # Generate realistic NDVI data with seasonal patterns
        ...
        # Highlight bloom periods
        bloom_months = [3, 4, 10, 11]  # April, May, November, December
...
def show_calendar_tab(farmer):
    ...
    advice = crop_calendar.get_agricultural_advice(selected_crop, selected_stage, st.session_state.language)
