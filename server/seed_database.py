...
CROPS_DATA = ALL_KENYA_CROPS

# Legacy crop data with detailed information (for backward compatibility)
DETAILED_CROPS_DATA = {
    'maize': {
        'name': 'Maize',
        'name_sw': 'Mahindi',
    ...
    'coffee': ALL_KENYA_CROPS.get('coffee', {}),
    'tea': ALL_KENYA_CROPS.get('tea', {}),
    'rice': ALL_KENYA_CROPS.get('rice', {}),
    'potatoes': ALL_KENYA_CROPS.get('potatoes', {}),
    'sugarcane': ALL_KENYA_CROPS.get('sugarcane', {})
}

# Merge detailed data with ALL_KENYA_CROPS
for crop_id, crop_data in DETAILED_CROPS_DATA.items():
    if crop_id in ALL_KENYA_CROPS:
        ALL_KENYA_CROPS[crop_id].update(crop_data)
...
SAMPLE_FARMERS = [
    ...
    },
    {
        ...
        'county': 'Nakuru',
        'farm_size': 12.0,
        'crops': ['wheat', 'maize', 'pyrethrum'],
        'language': 'sw',
        'location_lat': -0.3031,
        'location_lon': 36.0800,
        'sms_enabled': True,
        'registered_via': 'web'
    },
    {
    ...
    },
    {
        ...
        'county': 'Murang\'a',
        'farm_size': 4.0,
        'crops': ['coffee', 'macadamia', 'maize'],
        'language': 'en',
        'location_lat': -0.7221,
        'location_lon': 37.1520,
        'sms_enabled': True,
        'registered_via': 'web'
    },
    {
        ...
        'county': 'Uasin Gishu',
        'farm_size': 20.0,
        'crops': ['maize', 'wheat', 'dairy'],
        'language': 'en',
        'location_lat': 0.5143,
        'location_lon': 35.2698,
        'sms_enabled': True,
        'registered_via': 'ussd'
    },
    {
        ...
        'county': 'Kilifi',
        'farm_size': 7.5,
        'crops': ['coconut', 'cashew_nuts', 'mangoes'],
        'language': 'sw',
        'location_lat': -3.6309,
        'location_lon': 39.8493,
        'sms_enabled': True,
        'registered_via': 'web'
    }
]
...
def seed_crops(db):
    """Seed crops collection with all Kenya crops"""
    logger.info("Seeding crops collection...")
    
    crops_collection = db['crops']
    crops_collection.delete_many({})  # Clear existing
    
    for crop_id, crop_data in ALL_KENYA_CROPS.items():
        crop_doc = {
            'crop_id': crop_id,
            **crop_data,
    ...
    crops_collection.create_index('category')
    logger.info(f"✓ Seeded {len(ALL_KENYA_CROPS)} crops")

def seed_regions(db):
    """Seed regions collection with Kenya regions data"""
    logger.info("Seeding regions and counties collection...")
    ...
    for region_id, region_data in KENYA_REGIONS_COUNTIES.items():
        region_doc = {
            ...
            'name_sw': region_data['name_sw'],
            'counties': region_data['counties'],
            'created_at': datetime.now(),
            'updated_at': datetime.now()
        }
    ...
    logger.info(f"✓ Seeded {len(KENYA_REGIONS_COUNTIES)} regions with all 47 counties")
