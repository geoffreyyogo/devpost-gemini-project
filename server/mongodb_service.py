...
class MongoDBService:
    ...
    def register_farmer(self, farmer_data: Dict) -> Dict:
        ...
        try:
            ...
            # Create geospatial index for location-based queries (only if valid coordinates)
            if ('location_lat' in farmer_data and 'location_lon' in farmer_data and 
                farmer_data['location_lat'] is not None and farmer_data['location_lon'] is not None):
                farmer_data['location'] = {
                    'type': 'Point',
                    'coordinates': [farmer_data['location_lon'], farmer_data['location_lat']]
                }
            else:
                # Remove location fields if invalid to prevent geo index errors
                farmer_data.pop('location', None)
