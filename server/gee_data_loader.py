...
class GEEDataLoader:
    ...
    def load_kenya_data(self) -> Dict[str, np.ndarray]:
        ...
        NOTE: GeoTIFF loading DISABLED - we now use CSV data exclusively
        
        Why disabled:
          1. CSV has all statistics needed for ML and visualization  
          2. Old TIF files cause shape mismatch errors (7422x7422 vs 223x224)
          3. CSV data is from live Sentinel-2 fetches (10m resolution)
          4. Old TIF files are outdated (manual GEE exports, not current)
          5. ee_pipeline.py now provides fresh data via CSV
        
        Returns empty dict - all data comes from CSV processing now
        """
        data = {}
        
        # DISABLED: GeoTIFF loading causes errors and is outdated
        logger.info("GeoTIFF loading disabled - using CSV data from live Sentinel-2 fetches")
        logger.info("Old TIF files (landsat_ari, modis_ndvi) are outdated and cause shape mismatch errors")
        
        # Return empty - CSV processing in ee_pipeline.py handles all satellite data
        return data
        
        # OLD CODE DISABLED (kept for reference):
        # available = self.get_available_exports()
        # if available['modis_ndvi']:
        #     ndvi = self.load_geotiff(available['modis_ndvi'][0])  # Causes errors!
        # if available['landsat_ari']:
        #     ari = self.load_geotiff(available['landsat_ari'][0])  # Shape mismatch!
        # if available['ndvi_anomaly']:
        #     anomaly = self.load_geotiff(available['ndvi_anomaly'][0])  # Outdated!
